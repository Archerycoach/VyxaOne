...
Os handlers `runAnalysis` já estavam ligados nas páginas `src/pages/ai-organizer.tsx` e `src/pages/ai-performance-coach.tsx`. A falha confirmada nesta iteração é ausência do `Toaster` global em `src/pages/_app.tsx`, o que escondia totalmente os `toast(...)` de erro e sucesso.
