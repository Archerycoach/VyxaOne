...
interface CmaReportContext {
  ...
  /** €/m² dos comparáveis já HOMOGENEIZADOS (método comparativo). */
  homogenizedPricePerSqm?: number | null;
  /** Diferença % entre o homogeneizado e a média crua dos anúncios. */
  homogenizationDeltaPct?: number | null;
  homogenizationSampleSize?: number | null;
  /** Validação cruzada pelo método do custo. */
  costMethodMin?: number | null;
  costMethodMax?: number | null;
  /** Validação cruzada pelo método do rendimento. */
  incomeMethodValue?: number | null;
  incomeMethodYieldPct?: number | null;
...
` : ""}${context.homogenizedPricePerSqm ? `- COMPARÁVEIS HOMOGENEIZADOS (método comparativo): ${Math.round(context.homogenizedPricePerSqm)}€/m² — este é o valor TÉCNICO de referência. Cada comparável foi ajustado às condições concretas deste imóvel (fator negocial pedido→escritura, diferença de área pela fórmula p=pREF×(AREF/AOA)^¼, estado de conservação, idade, piso/elevador e vistas) ANTES de se tirar a mediana${context.homogenizationSampleSize ? `, sobre ${context.homogenizationSampleSize} referências` : ""}.${context.homogenizationDeltaPct ? ` A homogeneização alterou o valor em ${context.homogenizationDeltaPct > 0 ? "+" : ""}${context.homogenizationDeltaPct}% face à média simples dos anúncios — explica na secção de metodologia que comparar sem homogeneizar é o erro clássico da avaliação.` : ""}
` : ""}${context.costMethodMin && context.costMethodMax ? `- VALIDAÇÃO PELO MÉTODO DO CUSTO: ${Math.round(context.costMethodMin).toLocaleString("pt-PT")} € a ${Math.round(context.costMethodMax).toLocaleString("pt-PT")} € (terreno + construção + encargos + lucro do promotor). Usa como controlo de sanidade: se o valor recomendado ficar MUITO abaixo do custo de reposição, di-lo.
` : ""}${context.incomeMethodValue ? `- VALIDAÇÃO PELO MÉTODO DO RENDIMENTO: ${Math.round(context.incomeMethodValue).toLocaleString("pt-PT")} €${context.incomeMethodYieldPct ? ` (renda de mercado capitalizada a uma yield de ${context.incomeMethodYieldPct}%)` : ""} — a perspetiva do investidor.
...
4. "Metodologia e cálculo" — EM LISTA, as abordagens independentes que sustentam o valor: (a) MÉTODO COMPARATIVO com HOMOGENEIZAÇÃO — cada comparável ajustado às condições deste imóvel (fator negocial, área, estado, idade, piso/elevador, vistas) antes da mediana; é a abordagem principal; (b) comparativo pela ZONA/freguesia (€/m² de referência × área); (c) múltiplo do VPT (3,3–3,8×, típico na Área Metropolitana de Lisboa) — só quando há VPT; (d) MÉTODO DO CUSTO (terreno + construção + encargos + lucro) e (e) MÉTODO DO RENDIMENTO (renda × 12 / yield) — quando disponíveis, como validação. Uma linha por método, com o valor a que cada um chega, e uma frase final a dizer se convergem.
