...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const { data: users } = await supabase
      .from("profiles")
      .select("id, full_name, email, automation_paused, gpt_daily_email")
      .eq("gpt_daily_email", true)
      .eq("automation_paused", false);

    if (!users || users.length === 0) {
      console.log("No users with GPT daily email enabled");
    ...
    for (const user of users) {
      try {
        ...
        try {
          ...
          console.error(`Failed to parse AI planning for user ${user.id}:`, parseError);
          results.push({ userId: user.id, status: "failed", error: "Parse error" });
          continue;
        }
        ...
        if (!smtpSettings || !user.email) {
          console.log(`No active SMTP or email for user ${user.id}, skipping`);
          results.push({ userId: user.id, status: "skipped", reason: "No SMTP or email" });
          continue;
        }
...
Este email foi gerado automaticamente pelo Assistente IA do Vyxa CRM.
        ...
        console.log(`✅ Daily planning email sent to ${user.email}`);
        results.push({ userId: user.id, status: "success", email: user.email });
    ...
    console.error("GPT Assistant cron error:", error);
