...
import {
  ...
  saveSellerStages,
  type PipelineStage,
} from "@/services/pipelineSettingsService";
  ...
  return (
    ...
    >
      ...
      <div className="flex-1">
        <Label className="text-xs text-slate-600 mb-1">Nome da Fase</Label>
        <Input
          value={stage.name}
...
export function PipelineStagesSettings() {
  ...
  const fetchStages = async () => {
    setIsLoading(true);
    try {
      const [buyers, sellers] = await Promise.all([getBuyerStages(), getSellerStages()]);
      setBuyerStages(buyers);
      setSellerStages(sellers);
    } catch (error) {
      console.error("Error fetching pipeline stages:", error);
      toast({
        title: "Erro ao carregar fases do pipeline",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      await Promise.all([saveBuyerStages(buyerStages), saveSellerStages(sellerStages)]);

      toast({
        title: "✅ Fases guardadas",
        description: "As suas fases de pipeline de compradores e vendedores foram atualizadas.",
      });
    } catch (error: any) {
      console.error("Error saving pipeline stages:", error);
      toast({
        title: "Erro ao guardar",
        description: error.message || "Falha ao guardar as fases do pipeline.",
        variant: "destructive",
      });
    } finally {
      setIsSaving(false);
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          🎯 As Suas Fases do Pipeline
        </CardTitle>
        <CardDescription>
          Personalize as etapas do seu funil de compradores e vendedores. Estas fases são só
          suas — alterá-las não afeta os outros consultores da equipa.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-8">
        ...
        <div className="flex justify-end pt-4">
          <Button onClick={handleSave} disabled={isSaving} size="lg">
            ...
            <Save className="mr-2 h-4 w-4" /> Guardar Fases
          </Button>
        </div>
      </CardContent>
    </Card>
